...
try {
    ...
    $adminPatterns = "admin|manage|intune|endpoint|mdm|sccm|autopilot"
    ...
    $adminHits = $ctDomains | Where-Object { $_ -match $adminPatterns }
...
foreach ($name in $nameVariations) {
    foreach ($suffix in $azureSuffixes) {
        ...
        $fqdn = "$name$suffix"
        try {
            $dns = [System.Net.Dns]::GetHostEntry($fqdn) 2>$null
            if ($dns) {
                $resource = [ordered]@{
                    FQDN      = $fqdn
                    Name      = $name
...
$dorks | ForEach-Object { $_.SearchURL = "https://github.com/search?q=$([System.Web.HttpUtility]::UrlEncode($_.Query))&type=code" }
...
if ($summary.VPNEndpoints.Count -gt 0) {
