...
az rest --method GET \
  ...
  --query "value[0].{Score:currentScore, Max:maxScore}"
```
...
az rest --method GET \
  ...
  --query "value[].{Action:title, MaxScore:maxScore, Service:service}" -o table
```
...
```bash
az rest --method GET \
  --url "https://graph.microsoft.com/v1.0/directory/recommendations?\$filter=status eq 'active'" \
  --query "value[].{Title:displayName, Impact:impactType, Status:status}" -o table
```
...
```bash
az rest --method GET \
  --url "https://graph.microsoft.com/v1.0/directoryRoles/filterByRoleTemplateId(roleTemplateId='62e90394-69f5-4237-9190-012177145e10')/members" \
  --query "value[].{Name:displayName, UPN:userPrincipalName, Type:@odata.type}" -o table
```
...
az rest --method GET \
  ...
  --query "value[].{Role:displayName, Members:members[].displayName}" -o json
```
...
az rest --method GET \
  ...
  --query "value[].{Role:roleDefinitionId, Principal:principal.displayName, Type:principal.@odata.type}" -o table
```
...
```bash
az rest --method GET \
  --url "https://graph.microsoft.com/v1.0/roleManagement/directory/roleEligibilityScheduleInstances" \
  --query "value[].{Principal:principalId, Role:roleDefinitionId, Start:startDateTime, End:endDateTime}" -o table
```
...
az rest --method GET \
  --url "https://graph.microsoft.com/v1.0/roleManagement/directory/roleAssignments?\$expand=principal" \
  -o json | jq '[.value[] | {principal: .principal.displayName, role: .roleDefinitionId}] |
  group_by(.principal) | map(select(length > 1)) | .[] | {user: .[0].principal, roles: [.[].role]}'
```
...
az rest --method GET \
  ...
  --query "value[].{Name:displayName, State:state, GrantControls:grantControls.builtInControls}" -o table
```
...
az rest --method GET \
  --url "https://graph.microsoft.com/v1.0/identity/conditionalAccess/policies" \
  --query "value[?conditions.users.excludeUsers || conditions.users.excludeGroups].{Name:displayName, ExcludedUsers:conditions.users.excludeUsers, ExcludedGroups:conditions.users.excludeGroups}" -o json
```
...
az rest --method GET \
  ...
  --query "value[].{Name:displayName, Type:@odata.type, Trusted:isTrusted}" -o table
```
...
```bash
az rest --method GET \
  --url "https://graph.microsoft.com/v1.0/servicePrincipals?\$expand=appRoleAssignedTo" \
  -o json | jq '.value[] | select(.appRoleAssignedTo | length > 0) |
  {name: .displayName, appId: .appId, roles: [.appRoleAssignedTo[].appRoleId]}'
```

### App Registrations with Key/Password Credentials (secrets that expire)

```bash
az rest --method GET \
  --url "https://graph.microsoft.com/v1.0/applications?\$select=displayName,appId,passwordCredentials,keyCredentials" \
  --query "value[?passwordCredentials || keyCredentials].{Name:displayName, AppId:appId, Secrets:passwordCredentials[].displayName, Certs:keyCredentials[].displayName}" -o json
```
...
az rest --method GET \
  ...
  '.value[] | select(.passwordCredentials[]? | .endDateTime < $now) | {name: .displayName, expired: [.passwordCredentials[] | select(.endDateTime < $now) | .endDateTime]}'
```

### Apps with Owner-Less Registrations (orphaned apps)
...
az rest --method GET \
  ...
  --query "value[].{ClientId:clientId, Scope:scope, ConsentType:consentType}" -o table
```
...
az rest --method GET \
  ...
  --query "value[].{Name:displayName, Id:id, MemberCount:@odata.count}" -o table
```
...
```bash
az rest --method GET \
  --url "https://graph.microsoft.com/v1.0/groups?\$select=displayName,id" \
  -o json | jq -r '.value[].id' | while read id; do
...
### Dynamic Groups — Review Membership Rules for Overly Broad Queries

```bash
az rest --method GET \
  --url "https://graph.microsoft.com/v1.0/groups?\$filter=groupTypes/any(g:g eq 'DynamicMembership')" \
...
az rest --method GET \
  --url "https://graph.microsoft.com/v1.0/directoryRoles?\$expand=members" \
  -o json | jq '.value[] | {role: .displayName, groups: [.members[] | select(."@odata.type" == "#microsoft.graph.group") | .displayName]}'
```
...
### Users Registered for MFA (authentication methods registration)

```bash
az rest --method GET \
  --url "https://graph.microsoft.com/v1.0/reports/authenticationMethods/userRegistrationDetails" \
  --query "value[].{UPN:userPrincipalName, MfaRegistered:isMfaRegistered, Methods:methodsRegistered}" -o table
```
...
az rest --method GET \
  ...
  --query "value[].{UPN:userPrincipalName}" -o table
```

### Authentication Methods Policy (which methods are enabled tenant-wide)
...
### Legacy Authentication — Check Sign-In Logs for Basic Auth Usage

```bash
az rest --method GET \
  --url "https://graph.microsoft.com/v1.0/auditLogs/signIns?\$filter=clientAppUsed eq 'Exchange ActiveSync' or clientAppUsed eq 'Other clients'&\$top=50" \
  --query "value[].{User:userDisplayName, App:clientAppUsed, Status:status.errorCode}" -o table
```
...
az rest --method GET \
  ...
  --query "value[].{Name:displayName, Email:mail, Created:createdDateTime}" -o table
```
...
az rest --method GET \
  --url "https://graph.microsoft.com/v1.0/directoryRoles?\$expand=members" \
  -o json | jq '.value[] | {role: .displayName, guests: [.members[] | select(.userType? == "Guest") | .displayName]} | select(.guests | length > 0)'
```
...
az rest --method GET \
  ...
  --query "{GuestInviteSettings:allowInvitesFrom, GuestUserRole:guestUserRoleId}" -o json
```
...
```bash
az rest --method GET \
  --url "https://graph.microsoft.com/v1.0/users?\$filter=userType eq 'Guest'&\$select=displayName,signInActivity,createdDateTime" \
  -o json | jq --arg cutoff "$(date -u -d '90 days ago' +%Y-%m-%dT%H:%M:%SZ)" \
  '.value[] | select(.signInActivity.lastSignInDateTime < $cutoff or .signInActivity.lastSignInDateTime == null) | {name: .displayName, lastSignIn: .signInActivity.lastSignInDateTime}'
```
...
### Find Owner/Contributor Assignments at Subscription Level (should be rare)
...
az role definition list --custom-role-only true \
  --query "[].{Name:roleName, Actions:permissions[0].actions, DataActions:permissions[0].dataActions}" -o json
```

### Find Classic (Co-)Administrators — Deprecated, Should Be Removed

```bash
az role assignment list --include-classic-administrators \
  --query "[?roleDefinitionName=='CoAdministrator' || roleDefinitionName=='ServiceAdministrator'].{Principal:principalName, Role:roleDefinitionName}" -o table
```
...
az rest --method GET \
  ...
  --query "value[].{Device:deviceName, User:userDisplayName, OS:operatingSystem, State:complianceState}" -o table
```
...
az rest --method GET \
  ...
  --query "value[].{UPN:userPrincipalName, RiskLevel:riskLevel, RiskState:riskState, LastUpdated:riskLastUpdatedDateTime}" -o table
```
...
```bash
az rest --method GET \
  --url "https://graph.microsoft.com/v1.0/auditLogs/directoryAudits?\$filter=activityDisplayName eq 'Add member to role'&\$top=20" \
  --query "value[].{Activity:activityDisplayName, Actor:initiatedBy.user.userPrincipalName, Target:targetResources[0].displayName, Date:activityDateTime}" -o table
```
